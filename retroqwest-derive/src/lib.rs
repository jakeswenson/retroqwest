...
use syn::{
    ...
    Pat, PatIdent, PatType, Signature, TraitItem, TraitItemMethod,
};
...
impl TryFrom<Attribute> for HttpMethodAttribute {
    ...
    fn try_from(att: Attribute) -> Result<Self, Self::Error> {
        ...
        let path_seg = segments
            .next()
            .ok_or(syn::Error::new(att.span(), "http attribute missing method"))?;
...
fn get_att(attrs: &mut Vec<Attribute>, name: &'static str) -> Option<Attribute> {
    attrs
        .iter()
        .enumerate()
        .find_map(move |(i, a)| {
...
fn build_method(
    ...
    let attr = get_att(attrs, "http")
        .ok_or(syn::Error::new(sig.span(), "Missing http method attribute"))?;
    ...
    let uri_args = args.iter().filter_map(|a| match a {
        HttpArg::Uri { arg, span } => Some(quote_spanned!(*span=> #arg = #arg)),
        _ => None,
    });
    ...
    let body_args = args.iter().filter_map(|a| match a {
        HttpArg::JsonBody { arg, span } => Some(quote_spanned!(*span=> .json(#arg))),
        _ => None,
    });
...
fn expand(mut def: ItemTrait) -> Result<proc_macro2::TokenStream, syn::Error> {
    ...
    def.attrs
        .push(parse_quote!(#[retroqwest::async_trait::async_trait]));
